	...
	for {
		...
		case msg := <-cs.connCh:
			...
			if header.Height > MaxReorgLimit {
				...
				// Prune all hash data not updated in the past 20 minutes.
				...
				twentyMinutesAgo := time.Now().Add(-time.Minute * 20).UnixNano()
				err = cs.cfg.db.pruneHashData(twentyMinutesAgo)
				if err != nil {
